...
export class InputVolume extends SingletonAction<InputVolumeSettings> {
	...
	async onWillAppear(ev: WillAppearEvent<InputVolumeSettings>): Promise<void> {
		...
		await vMixInstance.registerInputVolume(ev.payload.settings.input, ev.action, this, ev.payload.settings);
	}
	...
	async onDidReceiveSettings(ev: DidReceiveSettingsEvent<InputVolumeSettings>): Promise<void> {
	...
	async onTouchTap(ev: TouchTapEvent<InputVolumeSettings>): Promise<void> {
		...
		this.setDialState(ev.action.id, state);
		await vMixInstance.toggleAudioBus(ev.payload.settings.input, state.sent);
		await ev.action.setFeedbackLayout("json/layouts/" + (sent ? "green" : "red") + ".json");
		await ev.action.setFeedback({
