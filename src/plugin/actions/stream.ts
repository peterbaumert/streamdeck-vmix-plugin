...
export class Stream extends SingletonAction<StreamSettings> {
	...
	async onWillAppear(ev: WillAppearEvent<StreamSettings>): Promise<void> {
		...
		await vMixInstance.registerStream(ev.action, this, ev.payload.settings);
	}
	...
	async initializeState(action: Action<StreamSettings>, settings: StreamSettings, override: boolean = false) {
		let changed = false;
