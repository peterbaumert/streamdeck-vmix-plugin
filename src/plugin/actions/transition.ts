...
export class Transition extends SingletonAction<TransitionSettings> {
	...
	async onWillAppear(ev: WillAppearEvent<TransitionSettings>): Promise<void> {
		...
		await vMixInstance.registerTransition(ev.payload.settings.input, ev.action, this, ev.payload.settings);
	}
	...
	async onDidReceiveSettings(ev: DidReceiveSettingsEvent<TransitionSettings>): Promise<void> {
	...
	async initializeState(action: Action<TransitionSettings>, settings: TransitionSettings, override: boolean = false) {
		let changed = false;
